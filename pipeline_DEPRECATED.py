# ============================================================
# DEPRECATED — Bug #14 fix: this file is an orphaned duplicate
# of pipeline/pipeline.py and is not imported anywhere in the
# codebase.  Do NOT edit this file.  The active pipeline is at:
...
class ParquetExporter:
    ...
    Exports processed video segments to Parquet in the format expected by r2dreamer's FPVDataset:
      - frames_rgb:     uint8 numpy array (T, H, W, 3) – RGB, masked
    ...
    Note: frames_gray is intentionally omitted.  r2dreamer's FPVDataset._build_raw_image()
    converts RGB → grayscale on-the-fly via frames.mean(dim=-1, keepdim=True), saving
    ~33 % storage and RAM per segment.
    """
    ...
    def export_segment(
        ...
        record = {
            ...
            # frames_gray omitted: trainer converts RGB→gray on-the-fly (saves ~33% storage)
            "frames_rgb":   [frames_rgb],                           # (T, H, W, 3) uint8
        ...
        while True:
            ...
            for chunk_start in range(0, n_window, chunk_size):
                ...
                for local_s, local_e in local_segments:
                    ...
                    is_terminal_seg, _, _ = crash_det.detect(seg_gray, flow_mags=flow_mags)
                    ...
                    del seg_512, seg_gray, seg_1080p, rgb_seg
...
def worker(worker_id: int, task_queue: multiprocessing.Queue, lock: multiprocessing.Lock, cfg: PipelineConfig, progress_counter: Optional[Any] = None, progress_lock: Optional[Any] = None) -> None:
    ...
    while True:
        ...
        for video_path, is_fpv, _ in _iter_url_videos(url, platform, tmp_dir, registry, logger):
            try:
                _process_video_cpu(video_path, is_fpv, parquet_exp, output_dir, cfg, logger, takeoff_det, osd_det, stick_det, scene_seg, crash_det, drone_cls, seg_registry, tmp_dir)
            except Exception as e:
