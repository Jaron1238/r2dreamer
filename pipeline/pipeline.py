...
class PipelineConfig:
    ...
    # 240 frames @ 512x288x3 (rgb, pre-JPEG) ≈ 106 MB raw — well under CI RAM budget.
    ...
    # SafetyNet trainer derives its grayscale input from frames_rgb
    # on-the-fly (resize up to safety_h/safety_w, then BGR2GRAY).
    jpeg_quality: int = 95
...
class ParquetExporter:
    ...
    Exports processed video segments to Parquet in the format expected by r2dreamer's FPVDataset:
      - frames_rgb:        bytes – JPEG-encoded frame stream at RSSM resolution
                           (out_h × out_w, default 288×512), one
                           [uint32 length][JPEG bytes] record per frame
                           (see _encode_rgb_jpeg). RGB, masked.
      - frames_rgb_shape:  int64[3] – (T, out_h, out_w) the JPEG stream decodes to
      ...
      - cam_overlay:    uint8 numpy array (H, W)              – binary overlay mask (0/255), RSSM resolution
    ...
    CHANGED vs. the old raw-bytes format:
      1. frames_gray is no longer written to the main FPV dataset at all.
         It used to duplicate a full frame at safety resolution
         (safety_h × safety_w, default 720×1280) purely for the SafetyNet.
         frames_rgb stays at RSSM resolution (out_h × out_w, 288×512) — the
         SafetyNet trainer now derives its input by cv2.resize'ing frames_rgb
         up to safety_h/safety_w (2.5x linear) and then BGR2GRAY, instead of
         getting a native-resolution copy. This is a real quality tradeoff
         (softened fine detail — thin branches, wires) traded for a large
         storage win; a dedicated higher-res column is the fix if SafetyNet
         recall regresses, not reinstating frames_gray.
      ...
      2. frames_rgb is JPEG-encoded (quality=CFG.jpeg_quality, default 95)
         ...
         frames_rgb, cv2.imdecode each, cv2.cvtColor(BGR2RGB) — that's the
         RSSM input directly; resize up to safety_h/safety_w + BGR2GRAY
         for SafetyNet.
    ...
    def export_segment(
        ...
        stem: str,
        segment_id: int,
        frames_rgb_chunks: List[np.ndarray],           # list of (t, out_h, out_w, 3) uint8 RGB chunks – already inpainted
...
def _process_video_cpu(video_path: Path, is_fpv: bool, parquet_exp: ParquetExporter, crash_exp: CrashDatasetExporter, output_dir: Path, cfg: PipelineConfig, logger: logging.Logger, takeoff_det: TakeoffDetector, osd_det: OSDAutoDetector, stick_det: StickCamDetector, scene_seg: SceneSegmenter, crash_det: CrashDetector, drone_cls: DroneClassifier, seg_registry: SegmentRegistry, tmp_dir: Path) -> None:
    ...
    try:
        ...
        open_bufs: Dict[str, list] = {
            # (T_chunk, out_h, out_w, 3) uint8 – RGB at RSSM resolution
            # (288x512 by default). No separate frames_gray buffer anymore:
            # the trainer upscales this to safety_h/safety_w + grayscale-
            # converts for SafetyNet input. seg_safety_gray is still computed
        ...
        def _flush_scene(sid: int) -> None:
            ...
            # frames_rgb is stored at RSSM resolution (out_hw_cam, 512x288),
            # not safety resolution — fall back to an empty mask at that
            # resolution if it hasn't been built yet.
        ...
        while True:
            ...
            for chunk_start in range(0, n_window, chunk_size):
                ...
                for local_s, local_e in local_segments:
                    ...
                    # Stored at RSSM resolution (seg_512, overlay_mask_cam) — this is
                    # now the ONLY RGB copy written to disk. The RSSM trainer uses it
                    # directly; the SafetyNet trainer upscales it to safety_h/safety_w
                    # (cv2.resize, 2.5x linear) and grayscale-converts, both at load
                    # time. That upscale is a real quality tradeoff vs. a native
                    # safety-resolution copy — fine detail (thin branches, wires) is
                    # softened. If SafetyNet crash/near-miss recall regresses, the
                    # fix is a dedicated higher-res store_h/store_w column, not
                    # reverting to a second full frames_gray column.
